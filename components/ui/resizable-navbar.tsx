...
export const Navbar = ({ children, className }: { children: React.ReactNode; className?: string }) => {
  ...
  return (
    ...
    >
      {React.Children.map(children, (child) => (React.isValidElement(child) ? React.cloneElement(child as React.ReactElement<any>, { visible }) : child))}
    </motion.div>
  );
};
...
export const NavItems = ({ items, className, onItemClick }: { items: NavItem[]; className?: string; onItemClick?: () => void }) => {
    ...
    >
      {items.map((item, idx) => (
        <a
          ...
          onClick={onItemClick}
          className={`${item.active ? "bg-gray-100" : ""} relative px-4 py-2 text-neutral-600 dark:text-neutral-300  rounded-full`}
          key={`link-${idx}`}
          href={item.link}
        >
