...
const NavbarFlow = ({
  ...
  const toggleSection = (text: string): void => {
  ...
  const renderSubmenuItems = (submenu: React.ReactNode): React.ReactNode => {
    if (!React.isValidElement(submenu)) return null;

    const submenuProps = submenu.props;
  ...
  return (
    <div className={`sticky top-0 z-50 w-full ${styleName}`}>
      <div className="hidden md:block">
        <div
          className="relative w-full max-w-7xl mx-auto h-24 flex items-center justify-between px-4 lg:px-10">
          <motion.div
            ...
            {links.map((element) => (
              <div key={element.text}>
                {element.submenu ? (
                  ...
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: sequenceDone ? 1 : 0 }}>
                    <a
                      href={element.url || "#"}
      ...
      <div className="block md:hidden">
        <div
          className="top-0 z-50 w-full border-b border-gray-200/40 dark:border-gray-800/40 bg-gray-50/95 dark:bg-black/95 backdrop-blur supports-[backdrop-filter]:bg-gray-50/60 dark:supports-[backdrop-filter]:bg-black/60 relative">
          <div className="container flex h-16 max-w-screen-2xl items-center px-4">
            <motion.div
              ...
              <div
                className="bg-gray-200/80 dark:bg-gray-800/80 backdrop-blur-sm text-gray-800 dark:text-gray-200 px-4 py-2 rounded-full font-semibold text-base">
                {emblem}
              </div>
            </motion.div>
